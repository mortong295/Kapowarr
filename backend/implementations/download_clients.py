...
class UsenetDownload(ExternalDownload, BaseDirectDownload):
    ...
    def __init__(
        ...
        LOGGER.debug('Creating Usenet download: %s', download_link)
        ...
        if not self._filename_body:
            self._filename_body = splitext(web_title or basename(download_link))[0]
    ...
    def update_status(self) -> None:
        ...
        if status.get('files'):
            self._files = [
                RemoteMappings.remote_to_local(self._external_client.id, f)
